...
def fetch_donotbuy_list(url: str, aliases: Dict[str, str]) -> pd.DataFrame:
    ...
    # Map locations
    df['FARMACIA'] = df['FARMACIA'].apply(lambda x: map_location(str(x), aliases) if pd.notna(x) else "")
...
def merge_donotbuy(df_sell_out: pd.DataFrame, df_donotbuy: pd.DataFrame, detailed: bool) -> pd.DataFrame:
    """
    ...
    if detailed:
        ...
        df_donotbuy_agg = df_donotbuy.sort_values('DATA', ascending=False).drop_duplicates('CNP', keep='first')
