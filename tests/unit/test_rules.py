...
from orders_master.formatting.rules import RULES, months_until_expiry, HighlightRule
...
def test_predicates():
    ...
    s_validade_longa = pd.Series({Columns.DTVAL: f"12/2099"})
...
def test_target_cells():
    df = pd.DataFrame(columns=[Columns.CODIGO, "X", "Y", Columns.T_UNI, Columns.PROPOSTA, Columns.DTVAL, Columns.PVP_MEDIO])
