...
def test_clean_designation_vectorized():
    ...
    expected_output = pd.Series(
        ["Ben-U-Ron 500Mg", "Acucar Mascavado", "Espacos", "Lowercase", "", "", "Valido 100%"]
    )
...
def test_remove_zombie_rows():
    ...
    df = pd.DataFrame({Columns.STOCK: [0, 1, 0, 10], Columns.T_UNI: [0, 0, 5, 10]})

    expected_df = pd.DataFrame(
        {Columns.STOCK: [1, 0, 10], Columns.T_UNI: [0, 5, 10]}, index=[1, 2, 3]
    )
...
def test_remove_zombie_aggregated():
    ...
    expected_df = pd.DataFrame(
        {Columns.CODIGO: [2, 2, 3], Columns.STOCK: [1, 0, 0], Columns.T_UNI: [0, 0, 5]},
        index=[2, 3, 4],
    )
