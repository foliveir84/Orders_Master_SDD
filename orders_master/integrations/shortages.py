...
@cache_decorator
def fetch_shortages_db(url: str, codigos_visible: Optional[Set[int]] = None) -> pd.DataFrame:
    """
    ...
    df['Data prevista para reposição'] = pd.to_datetime(df['Data prevista para reposição'], errors='coerce')
    ...
    if codigos_visible is not None:
        df = df[df['Número de registo'].astype(str).str.strip().isin([str(c) for c in codigos_visible])].copy()
