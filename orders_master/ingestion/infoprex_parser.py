...
def parse_infoprex_file(
    ...
    df = try_read_with_fallback_encodings(
        file_like, 
        sep='\t', 
    ...
    if 'CPR' not in df.columns or 'DUV' not in df.columns:
        raise InfoprexSchemaError(f"Colunas estruturais em falta no ficheiro {filename}. Esperado: CPR, DUV")
    ...
    if lista_codigos:
        df_filtered = df_filtered[df_filtered['CPR'].astype(str).str.strip().isin([str(c) for c in lista_codigos])]
    elif lista_cla and 'CLA' in df_filtered.columns:
        lista_cla_lower = [c.strip().lower() for c in lista_cla]
        df_filtered = df_filtered[df_filtered['CLA'].astype(str).str.strip().str.lower().isin(lista_cla_lower)]
    ...
    if Columns.STOCK in df_filtered.columns:
        df_filtered[Columns.STOCK] = pd.to_numeric(df_filtered[Columns.STOCK], errors='coerce').fillna(0).astype(int)
    if Columns.PVP in df_filtered.columns:
        df_filtered[Columns.PVP] = pd.to_numeric(df_filtered[Columns.PVP], errors='coerce').fillna(0.0)
    if Columns.P_CUSTO in df_filtered.columns:
        df_filtered[Columns.P_CUSTO] = pd.to_numeric(df_filtered[Columns.P_CUSTO], errors='coerce').fillna(0.0)
